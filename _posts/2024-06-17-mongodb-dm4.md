...
A suggestion is one doc per, device per day. Every new day a new document will be created. The doc contains an array per hour.
This easy-to-understand-for-human structure makes it difficult to average the temp for a given period => get the array or section for each hour of the period and do the calculation. Maybe a single array for a day makes more sense.
...
```
Another suggestion is have a document per device, per hour. There are 24 times more documents, but they are smaller. Beside, the temperatures are in an object with a minute key.
...
Another example when bucketing comes handy is storing message in a chat app with many channels.
...
**Problem**
- Avoiding too many documents or too big documents 1-* that can't be embedded.
...
We can have documents that have a property and other documents with another array property. Each will be annotated with the version of the schema that it conforms to.
...
Different collections involves more difficulties to find query results that come from both collections.
...
Another case of polymorphism occurs in embedded sub-documents. For example a customer with several addresses in different countries. Addresses are different by country, containing different properties.
...
For example, tracking page views can be achieved writing to the db each time a page is viewed, but it can be too many writes whenever there are hundreds of thousands visits a day.
